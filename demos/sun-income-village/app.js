(() => {
  ...
  const villages = {
    gaon: { name: '가온리 햇빛소득마을', short: '가온리', manager: '김운영', status: '운영 중', capacity: 200, members: 82, fullMembers: 76, payees: 72, review: 6, balance: 76450000, revenue: 7850000, expense: 1280000, generation: 21400, reconcileState: '통장 일치', reconcileDifference: 0, unsigned: 0 },
    deulkkot: { name: '들꽃리 햇빛소득마을', short: '들꽃리', manager: '이관리', status: '운영 중', capacity: 150, members: 67, fullMembers: 61, payees: 58, review: 6, balance: 48900000, revenue: 6120000, expense: 660000, generation: 16800, reconcileState: '통장 일치', reconcileDifference: 0, unsigned: 0 },
    solsaem: { name: '솔샘리 햇빛소득마을', short: '솔샘리', manager: '김운영', status: '준비 중', capacity: 100, members: 49, fullMembers: 41, payees: 39, review: 8, balance: 26350000, revenue: 0, expense: 4200000, generation: 0, reconcileState: '확인 필요', reconcileDifference: 120000, unsigned: 1 }
  };
  ...
  function renderDashboard() {
    ...
    $('#controlRows').innerHTML = keys.map((key) => {
      ...
      return `<tr><td><strong>${village.name}</strong><small>담당 ${village.manager}</small></td><td>${statusBadge(village.reconcileState)}<small>${difference ? `차이 ${won(difference)}` : '차이 0원'}</small></td><td class="money">${won(village.revenue)}</td><td class="money">${won(village.expense)}</td><td><span class="work-count ${approvalPending ? 'needs-attention' : ''}">${approvalPending}건</span></td><td><span class="work-count ${village.unsigned ? 'needs-attention' : ''}">${village.unsigned}건</span></td><td><div class="table-actions"><button type="button" data-open-village="${key}" data-open-view="accounting">회계</button><button type="button" data-open-village="${key}" data-open-view="approvals">결재</button></div></td></tr>`;
    }).join('');
